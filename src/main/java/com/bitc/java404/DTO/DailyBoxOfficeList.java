package com.bitc.java404.DTO;

import java.time.LocalDate;

public class DailyBoxOfficeList {
    private String rnum;
    private String rank;
    private String rankInten;
    private String rankOldAndNew;
    private String movieCD;
    private String movieNm;
    private String openDt;
    private String salesAmt;
    private String salesShare;
    private String salesInten;
    private String salesChange;
    private String salesAcc;
    private String audiCnt;
    private String audiInten;
    private String audiChange;
    private String audiAcc;
    private String scrnCnt;
    private String showCnt;

    public String getRnum() { return rnum; }
    public void setRnum(String value) { this.rnum = value; }

    public String getRank() { return rank; }
    public void setRank(String value) { this.rank = value; }

    public String getRankInten() { return rankInten; }
    public void setRankInten(String value) { this.rankInten = value; }

    public String getRankOldAndNew() { return rankOldAndNew; }
    public void setRankOldAndNew(String value) { this.rankOldAndNew = value; }

    public String getMovieCD() { return movieCD; }
    public void setMovieCD(String value) { this.movieCD = value; }

    public String getMovieNm() { return movieNm; }
    public void setMovieNm(String value) { this.movieNm = value; }

    public String getOpenDt() { return openDt; }
    public void setOpenDt(String value) { this.openDt = value; }

    public String getSalesAmt() { return salesAmt; }
    public void setSalesAmt(String value) { this.salesAmt = value; }

    public String getSalesShare() { return salesShare; }
    public void setSalesShare(String value) { this.salesShare = value; }

    public String getSalesInten() { return salesInten; }
    public void setSalesInten(String value) { this.salesInten = value; }

    public String getSalesChange() { return salesChange; }
    public void setSalesChange(String value) { this.salesChange = value; }

    public String getSalesAcc() { return salesAcc; }
    public void setSalesAcc(String value) { this.salesAcc = value; }

    public String getAudiCnt() { return audiCnt; }
    public void setAudiCnt(String value) { this.audiCnt = value; }

    public String getAudiInten() { return audiInten; }
    public void setAudiInten(String value) { this.audiInten = value; }

    public String getAudiChange() { return audiChange; }
    public void setAudiChange(String value) { this.audiChange = value; }

    public String getAudiAcc() { return audiAcc; }
    public void setAudiAcc(String value) { this.audiAcc = value; }

    public String getScrnCnt() { return scrnCnt; }
    public void setScrnCnt(String value) { this.scrnCnt = value; }

    public String getShowCnt() { return showCnt; }
    public void setShowCnt(String value) { this.showCnt = value; }
}
